...
ECASound.Track.prototype.volume = function(mVolume) {
    ...
    if (this.mVolume > this.iMaxVolume) {
        this.iMaxVolume = this.mVolume;
    }
...
ECASound.Track.prototype.fade = function(fTo, mDuration, mStart) {
    var fFrom    = this.mVolume;
    var iEffects = this.aEffects.length;
    if (iEffects > 0) {
        fFrom = this.aEffects[iEffects - 1].to;
    }
    
    // Amplification is Relative To the Maximum Volume
    // So if it's 20 and we want to get to 100, it's 100 / .2, which is 500
    /*
    if (fFrom > 0
    &&  fFrom < 100) {
        fTo = fTo / (fFrom / fTo);
        ...
        if (this.aEffects.length > 0) {
            for (var i in this.aEffects) {
                ...
                if (oEffect.type == 'fade') {
                    ...
                    aCommand.push(['-kl2:1', this.pproperty(oEffect.from), this.pproperty(oEffect.to), this.pproperty(oEffect.start), this.pproperty(oEffect.duration)].join(','))
                }
            }
        }
